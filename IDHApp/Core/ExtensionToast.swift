...
//  IDHApp
//
//  Created by boolean_wang on 2018/4/8.
...
extension UILabel {
    ...
    func getLableSize(text: String, maxWidth: CGFloat) -> CGRect {
        ...
        var size = text.boundingRect(with: maxSize, options: .usesLineFragmentOrigin,
...
class ToastView: NSObject {
    ...
    func showLoadingView() {
        ...
        container.backgroundColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 0.5022407017)  //半透明黑色背景
    ...
    func showLoadingDlg() {
        ...
        container.frame = CGRect(x: Int((rootRect?.width)!/2) - bgLength/2,
        ...
        indicatorView.frame = CGRect(x: (container.frame.size.width)/2 - indicatorLength/2,
                                     y: (container.frame.size.height)/2 - indicatorLength/2 - 10,
        ...
        //添加文字
        let lableX = (container.frame.size.width)/2 - CGFloat(bgLength/2) + 5
        let lableY = (container.frame.size.height)/2 + indicatorLength/2 - 10
