...
class HeatExchangerMonitorViewController: UIViewController, UITabBarDelegate {
...
//    @IBOutlet weak var monitorButton: UIButton!
    ...
    func getIndex() {
        for index in 0..<exchangersArr.count {
            if heatExchangerID == exchangersArr[index].ID{
                currentIndex = index
                break
            }
        }
    ...
    func showBackgroundAndTags(){
        ...
        //0:热源厂，1:换热站
        let url = "Http://\(idh_ip_port)/Analyze.svc/GetHoxConfig/\(heatExchangerID)/1"
    ...
    func setNavgationTitleAndButton(){
        if exchangersArr.count == 1 {
            preBtn.isHidden = true
            nextBtn.isHidden = true
        }else{
            if currentIndex == 0 {
                preBtn.isHidden = true
            }else if currentIndex == exchangersArr.count - 1{
        ...
        titleLabel.text = heatExchangerName
        //重新显示数据
...
extension HeatExchangerMonitorViewController:XMLParserDelegate{
